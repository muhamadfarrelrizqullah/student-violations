...
@section('title', 'Sanctions - EduGuard')
            ...
            <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    ...
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        ...
                        <li class="breadcrumb-item text-muted">Sanctions</li>
                    </ul>
                </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="tabelSanksi" class="table hover stripe" style="width: 100%">
                                        <thead>
                                            <tr>
                                                <th>No</th>
                                                <th>Name</th>
...
@section('modals')
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalDetailLabel">Detail Sanction</h5>
                ...
                <div class="modal-body">
                    <form>
                        ...
                        <div class="mb-3">
                            <label for="detailPoints" class="form-label">Points:</label></label>
                            <input type="number" class="form-control" id="detailPoints" readonly>
                        </div>
                    </form>
                </div>
    ...
    <div id="modalEdit" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modalEditLabel"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalEditLabel">Edit Sanction</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="formEdit" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="updatePoints" name="points"
    ...
    <div id="modalTambah" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modalTambahLabel"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTambahLabel">Add Sanction
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('datasanksi.store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="name" class="form-label">Name:</label>
...
@push('script')
    <script>
        document.getElementById('bt-download').addEventListener('click', function() {
            window.location.href = '/export-excel/sanction';
        });

        $(document).ready(function() {
            tabel = $('#tabelSanksi').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('datasanksi') }}",
                order: [[2, 'asc']],
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            return `<button type="button" onclick="modalDetail('${row.name}','${row.points}')"class="btn btn-primary btn-sm btn-icon-text"><i 
                                                    class="link-icon" data-feather="eye" data-bs-toggle="modal"
                                                    data-bs-target="#modalDetail"></i> </button>
                                    <button type="button" onclick="modalEdit('${row.id}','${row.name}','${row.points}')" class="btn btn-success btn-sm btn-icon-text"><i
        ...
        $(window).resize(function() {
            $('#tabelSanksi').DataTable().columns.adjust().responsive.recalc();
        });

        function modalDetail(name, points) {
            $('#detailName').val(name);
            $('#detailPoints').val(points);
        ...
        $('#modalTambah form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    Swal.fire(
                        'Error!',
                        'Error adding sanction: ' + xhr.responseJSON.message,
                        'error'
                    );
                }
            });
        ...
        function modalEdit(id, name, points) {
            $('#id').val(id);
            $('#updateName').val(name);
...
@push('style')
    <style>
        ...
        #tabelSanksi thead th:first-child {
            cursor: default;
        }

        #tabelSanksi thead th:first-child::after,
        #tabelSanksi thead th:first-child::before {
